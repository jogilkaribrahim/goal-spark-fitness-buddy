...
import { Activity, Target, Calendar, DollarSign, TrendingUp } from 'lucide-react';
...
export const FitnessGoalGenerator: React.FC<FitnessGoalGeneratorProps> = ({ data }) => {
  const generateFitnessPlan = () => {
    const { weight, targetWeight = weight, bmi, goal, duration = 60, budget = 0, currency = 'USD' } = data;
    ...
    const feasible = Math.abs(weightDifference) <= (isWeightLoss ? maxSafeWeightLoss : maxSafeWeightGain);
    ...
    for (let week = 1; week <= weeks; week++) {
      const weeklyGoal = isWeightLoss ? `Lose ${Math.abs(parseFloat(weeklyWeightChange))}kg` : 
                        isWeightGain ? `Gain ${parseFloat(weeklyWeightChange)}kg` : 
    ...
    for (let i = 1; i <= milestonesCount; i++) {
      ...
      milestones.push(`Week ${weekNumber}: ${expectedWeight.toFixed(1)}kg target`);
    }
    ...
    return {
      ...
      } ${feasible ? 'This goal is realistic and achievable!' : 'Consider extending your timeline for safer results.'}`,
      weeklyPlan,
  ...
  const getDietPlan = (goal: string | undefined, budget: number, currency: string, week: number) => {
    const budgetCategory = budget > 200 ? 'high' : budget > 100 ? 'medium' : 'low';
    ...
    return dietPlans[goal as keyof typeof dietPlans]?.[budgetCategory] || 
           'Balanced nutrition focusing on whole foods and appropriate portions';
  };

  const getWorkoutPlan = (goal: string | undefined, budget: number, week: number) => {
    const budgetCategory = budget > 200 ? 'high' : budget > 100 ? 'medium' : 'low';
    ...
    return workoutPlans[goal as keyof typeof workoutPlans]?.[budgetCategory] || 
           '30-45 mins mixed cardio and strength training';
  };

  const getWorkoutRecommendations = (goal: string | undefined, budget: number) => {
    if (budget < 50) {
  ...
  const getDietTips = (goal: string | undefined, isWeightLoss: boolean, isWeightGain: boolean) => {
    const baseTips = 'Stay hydrated with 8-10 glasses of water daily. Focus on whole foods and limit processed items.';
    
    if (isWeightLoss) {
      return baseTips + ' Create a moderate caloric deficit. Increase protein and fiber intake. Avoid liquid calories.';
    } else if (isWeightGain) {
      return baseTips + ' Increase caloric intake with healthy foods. Focus on protein for muscle building. Eat frequent meals.';
    } else {
      return baseTips + ' Maintain balanced macronutrients. Focus on nutrient-dense foods for overall health.';
    }
  };
      ...
      <div className="bg-gradient-hero text-white p-6 rounded-lg shadow-fitness">
        ...
        {!plan.feasible && (
          <div className="mt-4 p-4 bg-yellow-500/20 border border-yellow-500/30 rounded-lg">
            <p className="text-yellow-100 font-medium">⚠️ Consider a longer timeline for safer, more sustainable results</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-card p-6 rounded-lg shadow-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {plan.weeklyPlan.slice(0, 6).map((week) => (
            <div key={week.week} className="border border-border p-4 rounded-lg">
              <h4 className="font-semibold text-primary mb-2">Week {week.week}</h4>
